...
2. **The best model by raw accuracy (Random Forest) achieves roughly 71% test accuracy** (XGBoost close behind at 70%, though XGBoost leads on macro F1 — see §10.6) on a 4-class problem where the "always guess the majority-ish class" baseline is meaningfully lower but not negligible (see §10 for exact baseline maths). This is a reasonable but not deployment-grade result for real lending decisions.
...
**No explicit outlier detection or removal step exists for three of the four models** (no IQR filtering, no z-score capping). **XGBoost is now the exception**: its production preprocessing winsorizes every numeric ratio (clips to the 1st/99th training-percentile bounds, §5.7, §7.4) — the single largest individual accuracy gain found across the XGBoost notebook's 32-experiment search. Financial ratios are notoriously prone to extreme values — e.g., a company with near-zero equity produces an enormous `debtEquityRatio` or `companyEquityMultiplier` from a small denominator; a company with negative earnings and small revenue produces extreme profit margins. Tree-based models are generally **inherently robust to outliers** in a specific technical sense explained in §5.7 and §7 (splits only care about rank order, not magnitude) — which is why winsorization helping XGBoost specifically is worth understanding precisely: it's not about outliers confusing *splits*, it's about a handful of extreme values distorting *early boosting rounds'* gradient estimates, a boosting-specific mechanism, not a general tree-robustness one. Logistic Regression, by contrast, **is** sensitive to outliers (§7.1), which is a real, verifiable contributor to its still-weakest-of-four performance (48.0% accuracy, see §10.1) even after scaling was added.
...
## 6.3 Techniques for Correcting Imbalance — one reaches production (Decision Tree only), the rest don't

Verified by direct search: there is no import of `imblearn` (the standard Python library for SMOTE/oversampling techniques) and no manual oversampling/undersampling logic anywhere in `common.py`, `train_models.py`, or `predict.py`. **`class_weight` specifically is a mixed picture, not a clean "none of the models use it":** `class_weight=None` is passed explicitly for Random Forest and Logistic Regression, XGBoost has no equivalent parameter set at all, but **Decision Tree's estimator carries `class_weight="balanced"`** (§9.6) — a real, currently-deployed correction that doesn't appear in an earlier version of this section. It doesn't solve the problem (Distressed recall is still only 14% for Decision Tree, §6.4), but it is there. Separately, `notebook/xgboost/xgboost.ipynb`'s ablation study (§9.7) tested `sample_weight=compute_sample_weight("balanced", y_train)` for XGBoost in isolation, and it was the single best individual step in that study for macro F1 and Distressed-class F1 — but this finding has not been carried into `train_models.py`'s actual `XGBClassifier` construction, which still fits without `sample_weight`. This section explains what each of the standard corrective techniques is, so you can discuss them intelligently and explain precisely which of them production does and doesn't apply.
...
| ------------------- | ---------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------- |
| Decision Tree       | 3/22 = 14%                                     | 19% (3/16)            | Yes, 16 times total (13 wrong)                                                                       |
| Random Forest       | 2/22 = 9%                                      | 50% (2/4)             | Yes, 4 times total (2 wrong)                                                                          |
| Logistic Regression | 2/22 = 9%                                      | 33% (2/6)             | Yes, 6 times total (4 wrong)                                                                          |
| XGBoost             | 5/22 = 23%                                     | 62% (5/8)             | Yes, 8 times total (3 wrong) — best recall and precision of the four on this specific class            |

(All four numbers per model are computed directly from the raw confusion matrices in `models/training_summary.json`.)

Random Forest's 50% precision on Distressed is still a useful illustration of why a single metric misleads under imbalance: it only ever predicted "Distressed" 4 times in the entire 609-row test set, getting half right — a seemingly respectable-looking precision figure that's actually built on almost never predicting the rare class at all (catching only 2 of the 22 true cases). This exact table, and this exact interpretation, is one of the best "I understand my own results deeply" answers you can give an examiner who probes your evaluation section.
...
**Estimator used:** `sklearn.tree.DecisionTreeClassifier(random_state=42, criterion="gini", max_depth=None, max_features="log2", min_samples_split=2, min_samples_leaf=1, class_weight="balanced", splitter="random")`, fed through `build_preprocessor(..., scale_numeric=False)` — unlike an earlier version of this pipeline, scaling is *not* applied here (correctly — it would be harmless but pointless for a tree model, §5.5). `criterion="gini"` is scikit-learn's default (an earlier version of this project used `"entropy"`; that choice was since reverted). Three real, non-default choices are in the current config: `max_features="log2"` restricts each split to considering only a random subset of features (roughly $\log_2(25) \approx 5$ of the 25 available) rather than all of them, `splitter="random"` picks the best split among a set of *randomly chosen* candidate thresholds per feature rather than exhaustively searching every possible threshold, and **`class_weight="balanced"` reweights the impurity calculation so minority-class rows count for more** — this is a genuine class-imbalance correction, and its presence here means the blanket claim "no model in this project corrects for class imbalance" is not quite right; see §6.4's more precise framing. `max_depth=None` and `min_samples_leaf=1` remain scikit-learn's defaults — so the tree can still grow arbitrarily deep, which is directly relevant to the overfitting discussion below.
...
where $p_k$ is the proportion of class $k$ among the rows at that node. Gini impurity is 0 when a node is perfectly pure (all rows belong to one class) and increases toward its maximum as the classes become more evenly mixed. The alternative criterion, **entropy** (from information theory), measures the same underlying idea — how unpredictable the class label is at that node — via $-\sum_k p_k \log_2 p_k$, and choosing a split that most reduces entropy is called maximizing **information gain**. Gini and entropy usually select very similar splits in practice and rarely change the outcome substantially; scikit-learn defaults to Gini partly because it's marginally cheaper to compute (no logarithm). **This project's `DecisionTreeClassifier` uses `criterion="gini"`, scikit-learn's default** — an earlier version of this pipeline used `"entropy"` instead; that choice was since reverted back to the default, and (per the point above) the two would not typically be expected to change results dramatically on the same data regardless.
...
**Overfitting**, defined precisely, is when a model fits the training data's specific noise and idiosyncrasies so closely that it fails to generalize to new data — the gap between training performance and test performance widens. `DecisionTreeClassifier` here has no depth limit and no minimum-samples-per-leaf constraint (`max_depth=None`, `min_samples_leaf=1`), so the tree is still free to keep splitting until leaves contain very few (potentially single) training examples — `max_features="log2"` and `splitter="random"` add some regularization by restricting *what* each split can consider, but neither one limits *how deep* the tree can grow or *how small* a leaf can get, so the core overfitting risk remains. This is the textbook mechanism by which unconstrained decision trees overfit, and it is directly consistent with Decision Tree's mediocre 57.8% test accuracy despite being, structurally, the most flexible single model in the comparison (able to fit almost arbitrarily complex decision boundaries, unlike linear Logistic Regression).
...
At 57.8% accuracy — the second-worst of the four models — Decision Tree sits below both ensemble methods (Random Forest 71.3%, XGBoost 70.0%) that are explicitly designed to fix exactly the overfitting/high-variance weakness a single unconstrained tree suffers from (§7.3, §8), while still comfortably beating Logistic Regression's 48.0% because it can at least capture non-linear, threshold-based relationships. This ordering — single tree beats a linear model, but loses to tree ensembles — is exactly what standard ML theory predicts, and pointing this out explicitly is a strong signal you understand *why* your results look the way they do, not just *what* the numbers are.

---

## 7.3 Random Forest

**Estimator used:** `sklearn.ensemble.RandomForestClassifier(random_state=42, n_estimators=100, max_depth=20, max_features="sqrt", min_samples_split=5, min_samples_leaf=1, bootstrap=False, class_weight=None)`, fed through `build_preprocessor(..., scale_numeric=False)` — no scaling, correctly (harmless but unnecessary for a tree ensemble, §5.5). Only two values are genuine departures from scikit-learn's defaults: `max_depth=20` (default `None`/unrestricted, so each tree has an explicit depth cap) and `min_samples_split=5` (default 2, a mild regularizer requiring more rows before a node is allowed to split further). `n_estimators=100` and `max_features="sqrt"` are both scikit-learn's defaults, restated explicitly. **The one setting that matters most for how this model actually works is `bootstrap=False`** — a real, deliberate departure from the default (`True`) with consequences significant enough to read the next section carefully.
...
### How randomization is introduced — feature subsampling only, not bagging

Classic Random Forest gets its diversity from *two* independent sources of randomness. **This project's configuration only uses one of them** — a fact worth understanding precisely, not glossing over:

1. **Bootstrap aggregating ("bagging")** — classically, each tree trains on a random sample of the training rows drawn *with replacement*, so some rows appear multiple times in a given tree's training sample and others (statistically, about 37%) don't appear at all. **This project sets `bootstrap=False`, which turns bagging off entirely** — every one of the 100 trees is trained on the *exact same, full* training set. There is no per-tree row sampling happening here at all, despite "bagging" being the textbook explanation of how Random Forest works.
2. **Random feature subsampling** — at each split within each tree, only a random subset of the available features is considered as candidates for the best split. This project uses `max_features="sqrt"`, scikit-learn's default ($\sqrt{25} \approx 5$ of the 25 available features per split) — this is the *only* source of randomness diversifying the trees in this configuration, since bagging is off.

**Why this matters for how you should describe this model.** With `bootstrap=False`, this is not textbook bagging — it's closer to what's sometimes called an "Extra-Trees-adjacent" configuration in spirit (though not identical to `ExtraTreesClassifier`, which also randomizes split thresholds): all diversity between trees comes from *which features* each split is allowed to consider, not from *which rows* each tree sees. This is a real, defensible design choice (it means every tree gets the full training signal, at the cost of a weaker diversification mechanism than full bagging), but if asked "how does Random Forest introduce randomness here," the honest, correct, specific answer is "feature subsampling only — bagging is explicitly disabled in this configuration," not the generic bagging-and-feature-subsampling textbook answer.

### Why averaging many trees still reduces overfitting, without bagging

A single unconstrained decision tree has high **variance** — it fits training noise closely, and with `random_state` fixed, feature-subsampling randomness is the only thing that makes any two trees in this forest different (since every tree sees the same full training set, not a bootstrap resample). When you average the predictions of many trees that each made *different* early split choices (because each was restricted to a different random subset of candidate features at each node), the training-noise-driven errors specific to any one tree's particular split sequence tend to cancel out, while the genuine underlying signal — visible to every tree regardless of which random feature subset it got — reinforces itself. This is a real variance-reduction mechanism, just a narrower one than full bagging would provide (feature-subsampling diversity alone, without also varying which rows each tree sees). It's still enough to substantially outperform a single tree here: Random Forest's 71.3% accuracy substantially exceeds the single Decision Tree's 57.8%, using otherwise identical preprocessing and evaluation.
...
- Generally robust and a strong choice across many tabular ML problems even without extensive tuning — Random Forest's own hyperparameters here (`max_depth=20`, `min_samples_split=5`, `bootstrap=False`, §9) received comparatively light, undocumented adjustment relative to the dedicated ablation study behind XGBoost's tuning (§9.7), yet it still comes out as the best model overall on raw accuracy and weighted F1 (§10.1) — evidence that a simple, lightly-adjusted ensemble can be a genuinely strong baseline, not just a placeholder before "real" tuning.

### Limitations

- Loses the direct interpretability of a single tree or of Logistic Regression's coefficients — with 100 trees, there is no single readable rule list; feature importance is available but is a coarser, aggregate signal, not a full explanation of any individual prediction (this exact gap is what §11's SHAP analysis exists to fill).
- More computationally expensive to train and to run inference with than a single tree or a linear model, since it must build and query many trees — and with `bootstrap=False`, every tree also trains on the full dataset rather than a smaller bootstrap sample, so this project's configuration is *more* expensive than a typical bagged forest of the same size, not less.
- Can still struggle on very rare classes (§6.4's Distressed-recall table shows Random Forest catching only 2 of 22 true Distressed rows) — feature-subsampling-driven variance reduction (this configuration has no bagging to also contribute, see above) addresses overfitting, but does **not** address class imbalance; a model can be low-variance and well-averaged while still being systematically biased toward predicting common classes, because the imbalance lives in the training data and loss function, not in the tree-construction randomness.

### Why it performs well here, precisely

Random Forest reaches **71.3% accuracy and 0.7015 weighted F1 — the best of all four models on both metrics, ahead of even XGBoost (70.0% / 0.6908)** — driven largely by its ensemble structure correcting the single Decision Tree's overfitting, plus the additional trees and depth cap from its own hyperparameter adjustment (§9). This makes a strong, evidence-based talking point: *"ensembling alone recovered the largest single jump in performance in this comparison — from 57.8% (one tree) to 71.3% (a forest of trees) — which is a direct, textbook demonstration of variance reduction through bagging, and it's enough to edge out even XGBoost's more heavily, deliberately tuned result on raw accuracy — though not on macro F1, where XGBoost still leads (§10.6)."*

---

## 7.4 XGBoost (Introduced Here — Full Masterclass in §8)

**Estimator used:** `xgboost.XGBClassifier(random_state=42, eval_metric="mlogloss", n_estimators=324, max_depth=8, learning_rate=0.0811, subsample=0.8213, colsample_bytree=0.6685, min_child_weight=1, gamma=0.0091, reg_lambda=0.5099, num_parallel_tree=3, tree_method="hist", max_bin=16)`, fed through a **winsorized** preprocessor (`build_winsorized_preprocessor`, not the plain `build_preprocessor` the other three models use) — numeric ratios are clipped to their 1st/99th training-percentile bounds before the model sees them (§5.7). This is not a small, single hyperparameter choice like the other three models' — it's the final output of `notebook/xgboost/xgboost.ipynb`'s full 32-experiment search (§9.7–§9.8): winsorization (the single largest individual gain found), Optuna-tuned hyperparameters (in place of the grid searches used elsewhere in the notebook), `num_parallel_tree=3` (native mini-bagging inside each boosting round), and `tree_method="hist"` with a deliberately coarse `max_bin=16` as a mild additional regularizer. This supersedes an earlier, simpler production configuration that came directly from the notebook's "Experiment 7" `GridSearchCV` cell (5 parameters, no winsorization) — that search is still fully documented in §9.7 as the *first* documented tuning result in this project, but it is no longer what's actually deployed.

XGBoost achieves **70.0% accuracy and 0.6908 weighted F1 — second-best of all four models on both metrics, behind Random Forest (71.3% / 0.7015, §7.3)**, but it keeps the **best macro-F1 of all four models (0.6067 vs Random Forest's 0.5761)**, meaning XGBoost's performance is more evenly distributed *across* the four classes rather than being propped up almost entirely by strong performance on the large classes — a meaningful distinction explored fully with the macro-vs-weighted-F1 concept in §10, and the actual reason to prefer XGBoost over the higher-raw-accuracy Random Forest for this specific, imbalance-sensitive problem. Given the shared depth of material on gradient boosting, the complete treatment of *how* XGBoost works, why it differs fundamentally from Random Forest despite both being tree ensembles, and why it is the most widely used tabular ML algorithm in industry and competitions, is given its own dedicated chapter next.
...
**Random Forest (classically bagging; this project's configuration is feature-subsampling only — §7.3 explains why):** in the textbook version, builds many trees **independently and in parallel**, each on a different random bootstrap sample, and combines them by averaging/voting *after the fact*. Every tree is trying to solve the *same* problem from scratch, on a slightly different data sample; the ensemble's strength comes from averaging away each tree's independent random error. **This project's actual Random Forest has `bootstrap=False`**, so every tree sees the same full training set — the "independently and in parallel" structural description still holds, but the diversity source is feature subsampling only, not row sampling (§7.3 has the full explanation).
...
- Random Forest: reduces the Decision Tree's variance via averaging many trees, without meaningfully increasing bias — this is precisely why it improves on a single tree, though in this project's actual configuration (`bootstrap=False`) that diversity comes from feature subsampling alone, not classic row-level bagging (§7.3).
...
Both ensemble methods (Random Forest, XGBoost) land in a favourable region of this tradeoff relative to the two non-ensemble models, which is exactly what your project's results show (70–71% accuracy for both ensembles vs. 57.8% for Decision Tree and 48.0% for Logistic Regression).
...
models = {
    "decision_tree": DecisionTreeClassifier(
        random_state=42, criterion="gini", max_depth=None, max_features="log2",
        min_samples_split=2, min_samples_leaf=1, class_weight="balanced",
        splitter="random"),
    "random_forest": RandomForestClassifier(
        random_state=42, n_estimators=100, max_depth=20, max_features="sqrt",
        min_samples_split=5, min_samples_leaf=1, bootstrap=False, class_weight=None),
    "logistic_regression": LogisticRegression(
        random_state=42, max_iter=2000, C=100.0,
        class_weight=None, penalty="l2", solver="lbfgs"),
    "xgboost": XGBClassifier(
        random_state=42, eval_metric="mlogloss", n_estimators=324, max_depth=8,
        learning_rate=0.0811, subsample=0.8213, colsample_bytree=0.6685,
        min_child_weight=1, gamma=0.0091, reg_lambda=0.5099,
        num_parallel_tree=3, tree_method="hist", max_bin=16),
}
```

**Grid search, random search, and Bayesian optimization are all present in this codebase — inside `notebook/xgboost/xgboost.ipynb`, not `train_models.py` directly.** `RandomizedSearchCV` and `GridSearchCV` (§9.3–§9.4) were used in the notebook's earlier experiments (§9.7); a later extended search (§9.8) added a genuine Optuna trial loop (§9.5). **The configuration actually deployed in `train_models.py` today comes from that later, more extensive search** (winsorization + Optuna tuning + `num_parallel_tree=3` + `hist`/`max_bin=16`), not from the earlier `GridSearchCV` cell — §9.7 tells the story of the earlier search since it's still a fully documented, valid piece of methodology, but §9.8 is what's actually running in production. **Decision Tree, Random Forest, and Logistic Regression's hyperparameters are also no longer library defaults, but — unlike XGBoost — there is no corresponding documented search, notebook cell, or ablation study behind *why* these specific values were chosen** (§9.6 has the precise, current list for each model). If asked, the honest answer is: "XGBoost's hyperparameters come from a fully documented, 32-experiment search you can walk an examiner through in the notebook; the other three models' hyperparameters were set directly in `train_models.py` without an equivalent documented search — that asymmetry is itself worth naming as a methodology gap, not concealing."
...
## 9.5 Optuna and Bayesian Optimization — used in the notebook's extended search (§9.8), still absent from `requirements.txt`
...
**Update — Optuna is now genuinely used, in the notebook's extended search only.** The initial ablation study and Experiments 1–8 (§9.7) used only scikit-learn's `GridSearchCV`/`RandomizedSearchCV`, as described above. But the notebook's later extended search (Experiment 19 onward, §9.8) does use a real `optuna.create_study()` trial loop, scored across a repeated-split harness — and **its result is what's actually deployed in production today** (the final selected XGBoost pipeline, §7.4). `optuna` is installed and importable in this environment but is still **not listed in `requirements.txt`** — a real, easily-fixed reproducibility gap: a fresh `pip install -r requirements.txt` would not actually be able to re-run the notebook's later cells. The honest statement now is: *"Optuna is used, and its result is what's deployed — but the dependency list wasn't updated to reflect that, which is a documentation/packaging gap worth fixing before calling the project reproducible end-to-end."*
...
- **Decision Tree**: `criterion="gini"` (scikit-learn's default — an earlier version used `"entropy"`, since reverted), `max_features="log2"` and `splitter="random"` (both genuine, undocumented departures from default that add some split-level regularization), and **`class_weight="balanced"`** (a real class-imbalance correction — see the note below and §6.4's more precise framing). Still no `max_depth`/`min_samples_leaf` constraint — as discussed in §7.2, an explicit depth/leaf-size limit remains the single hyperparameter most likely to further improve this specific model, since it directly targets the overfitting that's the most plausible explanation for its still-middling 57.8% accuracy.
- **Random Forest**: `max_depth=20` (down from unrestricted) and `min_samples_split=5` (up from the default 2) are genuine, undocumented departures from scikit-learn's defaults. **`bootstrap=False` is the most consequential departure** — it disables row-level bagging entirely, so every tree trains on the full dataset and diversifies only via `max_features="sqrt"` feature subsampling (§7.3 covers why this matters for how you should describe the model). `n_estimators=100` and `max_features="sqrt"` are both left at their defaults.
- **XGBoost**: production no longer uses Experiment 7's simple 5-parameter configuration — it uses the notebook's full final selected pipeline (§7.4, §9.8): winsorized preprocessing, Optuna-tuned `learning_rate≈0.081`, `max_depth=8`, `n_estimators=324`, `subsample≈0.821`, `colsample_bytree≈0.668`, `min_child_weight=1`, `gamma≈0.009`, `reg_lambda≈0.510`, plus `num_parallel_tree=3` and `tree_method="hist"`/`max_bin=16`. Every one of these values traces to a specific, reproducible notebook experiment (§9.8's table), not a manually-chosen value — the most evidence-backed hyperparameter choice among all four production models in this project, more so even than before, since it now reflects a 32-experiment search rather than a single `GridSearchCV` cell.

**A correction worth being explicit about:** an earlier version of this guide stated that no model in this project performs any class-imbalance correction. That is no longer accurate — **Decision Tree's `class_weight="balanced"` is a real, currently-deployed correction.** It does not fully solve the problem (Distressed recall is still only 14% for Decision Tree, §10.1), and it is not present on the other three models, so the broader critique in §6.4/§16.1 (that imbalance handling is inconsistent and mostly absent) still stands — just not in the absolute "zero models correct for imbalance" form.
...
1. **The ablation study's "Hyperparameter tuning only" step (Section 2)** (`RandomizedSearchCV`, 40 candidate configurations, 5-fold stratified CV, scored on accuracy) searches 5 parameters (`n_estimators`, `learning_rate`, `max_depth`, `subsample`, `colsample_bytree`) in isolation, purely to measure how much hyperparameter tuning *alone* contributes relative to five other single-technique changes (class-imbalance handling via `sample_weight`, early stopping, `tree_method="hist"`, a scoring-objective ablation, and a final combination of the winning changes). Its winning configuration (`n_estimators=200, max_depth=8, learning_rate=0.1, subsample=0.7, colsample_bytree=1.0` in the most recent run) is **not** the configuration deployed in production.
2. **Experiment 7's "Hyperparameter Tuning" cell** (`GridSearchCV`, an exhaustive 2⁵=32-combination grid over `n_estimators`, `max_depth`, `learning_rate`, `subsample`, `colsample_bytree`, 5-fold stratified CV, scored on accuracy) found `n_estimators=200, max_depth=5, learning_rate=0.1, subsample=0.8, colsample_bytree=1.0` — this **was** hard-coded into `train_models.py`'s `XGBClassifier` for a period of this project, but has since been superseded: production now uses the full final selected configuration from the notebook's extended search (winsorization + Optuna tuning + `num_parallel_tree=3` + `hist`/`max_bin=16`, §9.8, §7.4), validated at 0.6995 accuracy / 0.6067 macro F1. Experiment 7 remains the first, simplest documented tuning result in this project's history and is still worth knowing precisely, but if asked "what hyperparameters does your deployed model actually use," the accurate answer is the §9.8 configuration, not this one.

**Why these two searches can legitimately disagree** (a fair question an evaluator might ask): different search method (random sampling of 40 combinations vs. exhaustive search of a smaller 32-combination grid), and `RandomizedSearchCV`'s own run-to-run non-determinism under `n_jobs=-1` parallelism (documented explicitly in `outputs/xgboost_ablation_summary.md` §13) — the ablation study's own report shows its Step 2 configuration changing between runs even with `random_state` fixed.

**Section 6 — a strict, single-variable ablation confirming the search objective doesn't matter here:** a later addition to the ablation study reruns Section 2's exact search (same grid, same `n_iter=40`, same CV) with only one change: `scoring="f1_macro"` instead of `"accuracy"`. In the most recent run, it found the **identical** best hyperparameters as Section 2 and so produced identical metrics (0.6798 accuracy / 0.5800 macro F1 for both). This is a genuine, verified negative result — on this specific 5-parameter grid, optimizing for accuracy vs. macro F1 selects the same winner — not an assumption. It's a good example to cite if an evaluator asks whether you test single-variable ablations properly, since it isolates exactly one change (the scoring metric) against the same shared baseline every other section uses.

**The ablation study's headline finding is itself an example of `RandomizedSearchCV` non-determinism, and you should present it that way:** the ablation study tests hyperparameter tuning (Section 2) and class-imbalance handling (Section 3, `sample_weight="balanced"`) both individually and combined (Section 7). In the most recent full run: tuning alone reached 0.6798 accuracy / 0.5800 macro F1, imbalance handling alone reached 0.6732 accuracy / **0.5890 macro F1 (its best result on the Distressed class specifically, F1 0.3158)**, and **combined, the two changes improved on both individually** — 0.6831 accuracy / **0.6063 macro F1** / **0.3750 Distressed F1**, the best result in the entire table on every metric. An *earlier* run of the exact same notebook code found the opposite: the combination *underperformed* both individual steps (an interaction effect). Both results are real and correctly measured — the difference is Section 2's non-deterministic search landing on a different hyperparameter configuration each time, which propagates into whether the combination happens to stack well with that specific configuration or not. **The correct, defensible statement to an evaluator is not "tuning and imbalance handling interact" or "they stack additively" — it's "whether they interact is itself unstable under this single-split methodology, which is exactly why the later winsorization/Optuna work (§9.8) switched to a 15-split repeated-evaluation harness before drawing conclusions."** That reframe — noticing your own methodology's limitation and explaining how later work addressed it — is a stronger answer in a viva than either one-off finding alone.
...
**What has *not* been carried from the notebook into production, and is worth naming proactively:** the `sample_weight`-based imbalance handling that was the ablation study's best individual step for Distressed-class performance is not in `train_models.py`'s `XGBClassifier` construction (§6.3, §6.4) — only the hyperparameter tuning was productionized. Given the run-to-run instability described above, naively adding `sample_weight` on top of the current tuned hyperparameters without re-validating on a repeated-split methodology is exactly the kind of change whose outcome (helps, hurts, or a wash) can't be predicted from a single run — a legitimate, evidence-grounded reason to flag this as "next experiment, validated properly," not "obvious missing feature."

## 9.8 Extended Search (Experiments 9–28) — Winsorization, Optuna, and Ensembling

This work goes further than §9.7 and lives on the same notebook (`notebook/xgboost/xgboost.ipynb`), past Experiment 8. **It is a research-branch exploration, not yet productionized** — `train_models.py` still builds its `XGBClassifier` from Experiment 7's configuration (§9.7), not from what follows. Be precise about this distinction if an evaluator asks what's actually deployed versus what was explored.

**Why it exists.** Experiments 7–10 (four single-split `GridSearchCV`/tuning passes) were shown not to generalize reliably run-to-run, motivating a switch to a **15-split repeated-evaluation harness** (`robustness_df`, 15 independent `train_test_split`s, `random_state` 1–15) so every subsequent claim is validated on mean performance across splits, not a single lucky/unlucky one.

**Winning lineage — four levers, found and combined in this order:**

1. **Winsorization** (Improvement 3) — clip each numeric ratio to its 1st/99th training percentile before fitting, removing the influence of a handful of extreme-denominator ratio values (e.g. `debtEquityRatio`, `freeCashFlowPerShare`) on early boosting rounds. The single largest individual gain found (+2.5pp accuracy on its own).
2. **Optuna-tuned hyperparameters** (Experiment 19) — hyperparameter search scored on mean accuracy across several held-out splits (not single-split CV), after the single-split `GridSearchCV` passes were shown not to generalize.
3. **`num_parallel_tree=3`** (Experiment 15) — XGBoost's native mini-bagging (several trees grown per boosting round, then averaged). Chosen over an external bootstrap-based ensemble (Experiment 12, rejected — see below) because it doesn't discard any unique training rows, which matters on a dataset this small (n≈2,029, ~1,420 training rows).
4. **`tree_method="hist"`, `max_bin=16`** (Experiment 23) — coarser split-point resolution as a mild additional regularizer, found once the earlier three levers were fixed.

Two independent hyperparameter searches (a broad joint search in Experiment 26, and a narrow local search centered on the leading candidate in Experiment 27) converged on this same combination without improving on it further — evidence this is a genuine, reproducible optimum for this dataset rather than a lucky search result, not just an assumption that individually-good pieces stack.

**Validated performance** (mean across the 15 independent splits, none of which were used to select the configuration):
...
**Experiments tried and rejected, each for a specific, verified reason** — all confirmed via the same 15-split methodology, not a single test split. This is worth being able to state fluently: a long list of *negative* results, each with a concrete mechanism for why it didn't help, is stronger evidence of rigor than a short notebook with only wins in it.

| # | What was tried | Why it seemed promising | Why it was rejected |
| --- | --- | --- | --- |
| 12 | Bagged ensemble (`BaggingClassifier`, 15 members, bootstrap resampling) | Borrow Random Forest's variance-reduction mechanism — a robustness check showed real split-to-split accuracy variance (std ≈0.018) for one XGBoost model | Bootstrap resampling discards ~37% of unique rows per base learner, starving the already-tiny Distressed class (~3.5% of rows) further; no reliable gain at ~15x training/inference cost |
| 13 | Seed ensemble (10 copies, no bootstrap, `random_state` only) | Fix Experiment 12's row-discarding problem while still reducing variance from XGBoost's internal stochastic sampling | Still no reliable improvement, at ~10x cost — the ensembling mechanism wasn't the missing piece |
| 14 | Monotonic constraints (`monotone_constraints` on directional ratios) | Encode financial domain knowledge (e.g. higher leverage should never improve a rating) as a regularizer | XGBoost applies one constraint direction per feature across *all* classes simultaneously — no notion that `RatingGroup` is ordinal, likely under-serving Distressed specifically |
| 16 | Winsorization threshold sweep (0.5/99.5 through 5/95 percentile) | Check whether the 1st/99th default (chosen, not tuned) was actually optimal | No alternative threshold reliably beat the original default — confirmed rather than changed the existing choice |
| 17 | `dart` booster (boosting-round dropout) | A regularization mechanism fundamentally different from anything else tried | No reliable difference, and slower at inference than the default `gbtree` |
| 18 | Domain composite risk ratio (engineered Altman-Z-score-style feature) | Tree models still must *discover* multi-ratio interactions from ~1,420 rows; literature blends ratios explicitly for distress prediction | No reliable difference — confirms this project's original assessment that feature engineering is low-value for tree models, which already learn interactions natively |
| 20 | SHAP-informed `feature_weights` (bias column subsampling toward important features) | Feed the SHAP section's explanatory output back into training | No reliable difference; also adds a dependency on one reference model's SHAP output, computed once and reused across all 15 splits |
| 22 | `interaction_constraints` (features grouped by financial domain) | Restrict spurious cross-domain interactions a tree might fit by chance on a small dataset | No reliable difference — restricting interactions blocked as much genuine signal as noise |
| 24 | Early stopping, retried on winsorization + tuned hyperparameters | The original ablation study (§9.7, Section 4) only tested early stopping against the raw baseline; the model had changed enough that the conclusion might not still hold | Same conclusion held: no reliable difference, and the validation slice it requires (15% of training data) costs more than it saves on an already-small dataset |
| 26 | Joint search over hyperparameters + `num_parallel_tree` + `max_bin` + `grow_policy` | The individually-best values (Experiments 15/19/23) were never searched *jointly* | Did not beat Experiment 25's already-found combination — the individual optima combined without loss, and the new `lossguide` growth policy added nothing |
| 28 | One-vs-Rest decomposition (4 independent binary models, per-class `scale_pos_weight`) | Give each class its own imbalance handling — something a shared multiclass objective (or Experiment 14's constraints) can't express | Did not outperform the flat multiclass model, at 4x training cost per split, with per-class probabilities that aren't jointly calibrated |
| 29 | SHAP-informed feature pruning — drop the `Sector` one-hot dummies (all 12), which cluster at the bottom of SHAP importance below every numeric ratio | Low-signal columns on ~1,420 training rows are a plausible overfitting source; the SHAP section's own ranking flagged `Sector` as the obvious candidate | Lost on **15/15** splits of the robustness harness (mean accuracy 0.6656 vs 0.6796 with `Sector` kept) — low per-dummy SHAP value didn't mean the categorical feature as a whole was safe to drop; the signal was split across all 12 columns collectively |
| 30 | Replace one-hot `Sector` with a single smoothed ordinal target-encoded `SectorRiskScore` column (fit per training fold only, no leakage) | Experiment 29 showed `Sector`'s signal is real but diluted across 12 sparse dummies; concentrating it into one dense feature might let the model use it more effectively | No reliable difference across the 15-split harness (mean accuracy 0.6797 vs 0.6796, macro F1 +0.24pp) and won only 8/15 splits on accuracy — not worth the added encoding complexity over a standard `OneHotEncoder` |
| 31 | Sector-relative ratios — replace each numeric ratio with `ratio − sector_median_ratio` (sector medians from the training fold only), keeping one-hot `Sector` alongside | Leverage/liquidity norms differ a lot by industry; a company's standing relative to its peers may carry more signal than the raw absolute ratio | Accuracy exactly tied with the raw-ratio baseline (0.6796 vs 0.6796, won only 6/15 splits), though macro F1 moved more than any other Sector experiment (+0.94pp) — not adopted for the accuracy-optimized final model, but the largest macro F1 signal found among Experiments 29–31 if a future iteration prioritizes Distressed recall over accuracy |
| 32 | Ordinal regression reformulation — `XGBRegressor` predicting a continuous rank (0–3) instead of multiclass softmax, rounded/clipped to the nearest class | `RatingGroup` is genuinely ordinal and the confusion matrix's dominant errors are adjacent-class confusions; softmax treats all four classes as unordered | **Decisively rejected** — lost on all 15/15 splits on both metrics (accuracy −5.76pp, macro F1 −6.89pp). A naive round-to-nearest regression collapses toward the dense middle classes and under-predicts Distressed even more than softmax does; a proper cumulative-link ordinal model might behave differently but wasn't attempted |
...
| ------------------- | ---------- | ----------- | ---------- |
| Decision Tree       | 0.5780     | 0.5745      | 0.4793     |
| Random Forest       | **0.7126** | **0.7015**  | 0.5761     |
| Logistic Regression | 0.4795     | 0.4617      | 0.3799     |
| XGBoost             | 0.6995     | 0.6908      | **0.6067** |

All four numbers for every model are computed identically, by `evaluate_predictions()` in `common.py`, on the **same held-out 609-row stratified test set** (§5.8) — this consistency is what makes the comparison meaningful, and is worth stating explicitly if asked "is this a fair comparison across models." **No single model wins on every metric.** Random Forest leads on raw accuracy and weighted F1; XGBoost leads on macro F1 — the metric that weights all four classes equally rather than letting the large classes dominate, which is the more relevant metric for this project's actual imbalanced-classes problem (§10.6). Which model is "best" therefore depends on which metric you consider most important, and you should be ready to state that explicitly rather than name a single winner unqualified.
...
**Why this distinction is the single most important evaluation concept for this specific project.** Look again at the results table: Random Forest's macro F1 (0.5761) is noticeably *lower* than its weighted F1 (0.7015) — a gap of 0.125. XGBoost's gap is smaller (0.6067 vs 0.6908, a gap of 0.084), but still substantial. **This gap is direct, quantitative proof that both models perform considerably worse on the rare classes than on the common ones** — weighted F1 is inflated by strong performance on Speculative/Investment-Low/Investment-High (which dominate the support-weighted average), while macro F1 exposes the true, unweighted picture including the poor Distressed performance. **This is also the specific, principled reason to prefer XGBoost over Random Forest despite Random Forest's higher raw accuracy and weighted F1 (§10.1)** — XGBoost's performance is more evenly spread across classes, which is precisely the property you'd want in a genuinely fair credit-risk model, and its macro F1 (the metric that doesn't let the large classes hide the Distressed-class weakness) is the best of all four models. This is one of the highest-value insights in this entire document — it's the actual, defensible answer to "why did you pick XGBoost when Random Forest scores higher on accuracy," so make sure you can explain it fluently and unprompted.
...
*"No single model wins on every metric. Random Forest leads on raw accuracy and weighted F1 — 71.3% accuracy, 0.7015 weighted F1 — with XGBoost close behind at 70.0% / 0.6908, and both ensembles meaningfully outperforming a single Decision Tree (57.8%) and Logistic Regression (48.0%, which is now above the naive majority-class baseline of 39%, though it remains the weakest model overall). But XGBoost leads on macro F1 (0.6067 vs Random Forest's 0.5761), which is the metric I'd actually weight most heavily here, since it treats all four classes equally rather than letting the large classes dominate — and this project's whole point is correctly assessing the minority Distressed class, not maximizing an aggregate score. XGBoost's macro F1 lead is driven by a smaller macro/weighted F1 gap (0.084 vs Random Forest's 0.125), meaning its performance is more evenly spread across all four rating buckets, not just propped up by the large classes — and its hyperparameters are also the one set in this project backed by a fully documented, 32-experiment search (§9.7–§9.8), which is part of why I'd deploy it over Random Forest despite the lower raw accuracy. That said, every single model — including XGBoost — still performs poorly on the Distressed class specifically, which is exactly the class a real lender would care most about: XGBoost's own ablation study found that `sample_weight`-based class-imbalance handling was actually the single best individual technique for that class, but that finding hasn't been carried into the production model, and whether combining it with the current tuned hyperparameters would help is itself unstable across runs of the same single-split methodology (§9.7) — a properly validated answer would need the repeated-split approach used later in the same notebook (§9.8). I also did not compute ROC-AUC, calibration metrics, or a cost-sensitive evaluation, all of which would give a more complete picture of whether this model is actually fit for a real lending decision, as opposed to fit for a standard multi-class ML benchmark."*
...
- `train_single_model(...)` — builds one model's full `Pipeline` (preprocessor + estimator, §5.4), fits it on the training split, evaluates it on the test split, builds and saves its artifact, and returns a summary dict of its results. Called once per model inside a loop in `main()`. Accepts an optional `preprocessor` override — used for XGBoost only, to substitute the winsorized preprocessor (§5.7, §7.4) in place of the plain `build_preprocessor` the other three models use; every other model falls back to the default.
...
**Possible improvements:** no cross-validation used for the final reported numbers (§5.8, though the notebook's `GridSearchCV`/`RandomizedSearchCV` do use it internally, §9), hyperparameter tuning is fully documented for XGBoost only, not the other three models (§9.7), imbalance correction is inconsistent — only Decision Tree carries `class_weight="balanced"` into production, and even that doesn't fully solve the problem (§6.4, §9.6) — training and evaluation logic for all four models is duplicated in structure (though correctly factored through the shared `train_single_model` function) rather than run in parallel (they run sequentially in a `for` loop, which is fine at this dataset size but wouldn't scale gracefully to a much larger dataset or many more model types without parallelization).
...
**Random Forest** — classically a bagging-based ensemble of decision trees; this project's actual configuration uses feature subsampling only, `bootstrap=False` (§7.3).
...
15. **Explain the difference between macro and weighted F1 using your own numbers.** XGBoost's macro F1 (0.607) vs weighted F1 (0.691) — the 0.084 gap shows performance is markedly worse on rare classes than the weighted average suggests (§10.6).
16. **What is statistical significance, and did you test for it between your models?** A formal significance test (e.g., McNemar's test comparing paired predictions) was not performed; the ~1.3 percentage-point accuracy gap between Random Forest and XGBoost (and the reverse-direction macro F1 gap) has never been formally checked against what could be sampling noise from a single 609-row test split — a fair, honest limitation to name, particularly since the two models trade the lead depending on which metric you look at.
...
22. **What is variance, in the bias-variance sense, and which of your models has the most of it?** Sensitivity to training-sample noise; the unconstrained single Decision Tree has the highest variance of the four models, visibly corrected by Random Forest's tree-averaging — via feature subsampling in this project's actual `bootstrap=False` configuration, not classic row-level bagging (§7.2, §7.3, §8.7).
...
32. **How does Random Forest ensure its trees are different from each other?** Classically, bootstrap row sampling (bagging) plus random feature subsampling at each split — but this project's actual configuration has `bootstrap=False`, so only random feature subsampling at each split is active; there is no row-level sampling happening (§7.3).
...
50. **How would you extend this project to support additional sectors or a different country's rating conventions?** The 24 ratio features and preprocessing are generic; the main extension points are `SECTOR_CATEGORIES` (§3.4) needing new entries and `RATING_GROUP_MAPPING` (§3.6) needing a new letter-grade scheme, both centralized in `common.py` rather than scattered.
...
55. **Which of your four models is "the best," and why?** Depends on the metric — Random Forest leads on raw accuracy (71.3%) and weighted F1 (0.7015), but XGBoost leads on macro F1 (0.607 vs 0.576), which better reflects balanced performance across all four classes and is the more relevant metric for this project's imbalanced-classes problem (§10.6, §10.12). XGBoost's hyperparameters are also the one set in this project backed by a fully documented, 32-experiment search (§9.7–§9.8), which is a secondary reason to prefer it despite the lower raw accuracy.
...
66. **Random Forest shows 50% precision on Distressed — is that a good sign?** No — it only ever predicted Distressed 4 times in 609 rows, catching just 2 of the 22 true cases (9% recall); with so few predictions, a moderate-looking precision reflects extreme caution/avoidance, not reliable detection (§6.4).
...
96. **How confident are you that Random Forest's accuracy lead, or XGBoost's macro F1 lead, would hold on a different random split?** Not fully — no statistical significance test was run, and the ~1.3-point accuracy gap and ~3-point macro F1 gap are both plausibly within single-split sampling noise; cross-validation would give a more defensible answer (§10.1, evaluator question 16).
...
102. **Can you defend every number in your results table without looking it up?** You should be able to state, from memory: Decision Tree 57.8%, Random Forest 71.3%, Logistic Regression 48.0%, XGBoost 70.0% accuracy; and that Random Forest leads on accuracy/weighted F1 while XGBoost leads on macro F1 (0.607) — practice this cold (§10.1).
...
- **Class-imbalance correction is inconsistent and mostly absent from production** — only Decision Tree carries `class_weight="balanced"` (§9.6), and even that doesn't solve the problem: every model still catches well under half of true Distressed companies (9–23% recall across all four, §6, §6.4), including Decision Tree itself (14% recall despite the correction). XGBoost specifically has no imbalance handling, despite its own notebook's ablation study finding a working fix for it (`sample_weight="balanced"`, the single best individual step for Distressed F1, §9.7). *Risk:* this is the most damaging single technical gap given the project's stated business purpose is specifically credit *risk* — the part of the problem the project is weakest at is the part that matters most, and unlike hyperparameter tuning, the evidence that a fix works for XGBoost is already sitting in your own notebook, unused; Decision Tree's partial fix also demonstrates that `class_weight="balanced"` alone is not sufficient on this dataset, which is itself a useful, evidence-backed thing to say if asked "why not just add it everywhere." *Fix:* carry `sample_weight`/`class_weight` into the other three models' construction, validated properly via a repeated-split methodology rather than a single train/test split, since §9.7 shows a single split can't reliably tell you whether combining imbalance handling with existing tuning helps or hurts.
...
- **No statistical significance testing between models** — Random Forest's ~1.3-percentage-point accuracy edge over XGBoost, and XGBoost's own macro-F1 edge over Random Forest (0.607 vs 0.576), have never been formally checked for whether either gap is distinguishable from single-split sampling noise (§10.1, evaluator Q16, Q96). Given how close both gaps are, this matters more here than in a project with a clear runaway winner. *Fix:* McNemar's test on paired predictions, or bootstrap confidence intervals on the accuracy/F1 difference, would let you state each gap's significance rather than its size alone.
...
**Bootstrap**: sampling *with replacement* from a dataset to create a new sample of the same size — the mechanism behind *classic* Random Forest bagging (§7.3; this project's actual Random Forest has `bootstrap=False`, so this mechanism is not currently active in production, though it's still essential general ML vocabulary). A useful, examinable fact: for a bootstrap sample of size $n$ drawn from $n$ original rows, the probability any single specific row is *never* selected is $(1-\frac{1}{n})^n \to e^{-1} \approx 0.368$ as $n$ grows — which is where the commonly-cited "~37% of rows are left out of each bootstrap sample" figure comes from mathematically, not as an approximation pulled from nowhere. Those left-out rows for a given tree are called **out-of-bag (OOB)** samples, and can be used to estimate generalization performance without a separate held-out set — scikit-learn's `RandomForestClassifier` supports this via `oob_score=True`, but **that option requires `bootstrap=True` to mean anything (scikit-learn raises an error if you set it with `bootstrap=False`)** — so enabling OOB scoring in this project would first require re-enabling bagging itself, not just flipping one flag, which is a more consequential change (§7.3) than it might first appear.

## 18.11 Confidence Interval

A range around a point estimate (e.g., "accuracy = 70.0%") that, under repeated sampling, would contain the true underlying value a specified percentage of the time (e.g., a 95% confidence interval). **This project reports single-number point estimates only** (§10.1) — there is no confidence interval anywhere, which directly relates to §16.2's "no statistical significance testing" critique. A **bootstrap confidence interval** — repeatedly resampling the 609-row test set with replacement, recomputing accuracy each time, and taking the 2.5th/97.5th percentiles of that distribution — is the most concrete, implementable fix, and is exactly what §16.2's *Fix* line already proposes.
...
**Credit-level answer:** states *what* and *why*, with a specific number. *"I used XGBoost because it reached the best macro F1 of my four models (0.607), and handles the non-linear relationships Logistic Regression's 48.0% accuracy suggests it's missing."* Grounded in real numbers, but still comparing headline stats only.

**Distinction-level answer:** states *what*, *why*, cites the *mechanism*, and preempts the obvious follow-up. *"Random Forest actually edges out XGBoost on raw accuracy (71.3% vs 70.0%) and weighted F1, but XGBoost leads on macro F1 (0.607 vs 0.576) — the metric that treats all four rating classes equally rather than letting the large classes dominate, which is the metric that actually matters for a project whose stated purpose is assessing risk in a severely imbalanced problem. That lead is also the one in this project backed by a fully documented, 32-experiment search rather than an untraced hyperparameter choice, which is why I trust it more than the raw numbers alone would justify."* Mechanism-aware, ties multiple metrics together, and volunteers the caveat (Random Forest actually wins on accuracy) before being asked — that's a stronger move than picking the metric that makes your chosen model look best and staying quiet about the rest.

**High-Distinction-level answer:** does all of the above, *and* names the boundary of the claim unprompted. *"I chose XGBoost over the higher-accuracy Random Forest specifically because macro F1 is the more decision-relevant metric here — but I haven't run a formal significance test — McNemar's or a bootstrap CI — so I can state the size of both gaps precisely but not yet their statistical significance, and I'd want that before treating either 'XGBoost is best on macro F1' or 'Random Forest is best on accuracy' as more than the best evidence-supported read available from a single test split. I'd also flag that XGBoost's tuning search wasn't run for the other three models, so the whole four-way comparison isn't judging each model at its own best — only XGBoost's."* This answer is *more* impressive for stating the limitation, not despite it — it demonstrates the candidate understands the epistemic status of their own claim, which is precisely what separates genuine mastery from well-rehearsed talking points.

**What makes an answer sound memorized, even if factually correct:** reciting a number without being able to explain where it came from if asked a follow-up ("why 0.607 specifically — walk me through how that's computed from your confusion matrix"); using this document's exact phrasing verbatim without being able to paraphrase it differently on request; answering a slightly-reworded version of a rehearsed question with visible confusion rather than adapting.
...
**"What causes data leakage that isn't already covered in your leakage section?"** Beyond §5.9's named risks: **preprocessing-statistics leakage via hyperparameter search** — if a `GridSearchCV`'s cross-validation folds were constructed *after* a global preprocessing step fit on the whole training set (rather than the imputer/scaler being refit per-fold inside the `Pipeline`, as this project correctly does, §5.9), each fold's validation score would be silently contaminated.

**"Your `Distressed` recall is worse for XGBoost's raw hyperparameter-tuned version than for the class-imbalance-handling version alone (§9.7) — doesn't that mean tuning is actively harmful for the minority class?"** No — this is a subtly wrong inference the numbers invite but don't support. Tuning-alone (Step 2) optimizes for *accuracy* (its search's `scoring` argument), not Distressed recall specifically, so it's unsurprising it doesn't match a change (`sample_weight`, Step 3) built specifically to help the minority class. The correct reading, per §9.7, is that whether combining the two changes helps or hurts is itself unstable across runs of the same code (a `RandomizedSearchCV` non-determinism effect) — the honest answer is "it depends on the run, which is exactly why a single-split ablation study can't settle this on its own; a repeated-split methodology, as used later in the same notebook (§9.8), is what would."
...
flowchart TB
    ...
    IMBAL -->|"tested for XGBoost, not carried in;\nDecision Tree has class_weight='balanced' instead"| SW["sample_weight / class_weight ablation (§6.3, §9.6, §9.7)"]
    MODELS --> TUNE["Hyperparameter tuning (§9)\nGridSearchCV/RandomizedSearchCV (§9.7) +\nOptuna extended search (§9.5, §9.8) -- XGBoost only"]
    SW -.->|"whether they interact is\nrun-dependent, not settled (§9.7)"| TUNE
...
**To a data scientist.** "Four-class classification on 25 features (24 numeric ratios, 1 one-hot-encoded categorical), 2,029 rows, stratified 70/30 split. XGBoost's production hyperparameters come from a 32-experiment search — winsorization, Optuna-tuned parameters, native mini-bagging via `num_parallel_tree`, and a coarsened histogram method — not the simpler `GridSearchCV` result that ran in an earlier version of this project; the other three models' current hyperparameters are non-default but undocumented. An ablation study isolated hyperparameter tuning from `sample_weight`-based class reweighting and found that whether combining them helps or hurts is itself unstable across runs of the same single-split search methodology — one run showed them interacting negatively, a later run showed the combination beating both individually, which is itself the single most methodologically interesting finding in the project: it's evidence for why the later, extended search moved to a repeated-split evaluation harness instead of trusting any one split's conclusion. Random Forest actually leads on raw accuracy and weighted F1; XGBoost leads on macro F1, the more decision-relevant metric for this imbalanced problem — no single model wins on every metric. No calibration, no formal significance testing, no cross-validation on the final reported numbers — all named explicitly as scope limitations, not hidden."
...
13. What's XGBoost's test accuracy? → 70.0% (§10.1).
...
22. Why does macro F1 penalize Random Forest more than XGBoost, relatively speaking? → Random Forest's larger macro/weighted gap (0.125 vs. XGBoost's 0.084) reflects worse relative minority-class performance (§10.6).
...
26. If you retrained today with `sample_weight` added to XGBoost's current tuned hyperparameters, would you expect improvement — and how confident are you? → Not confidently — §9.7's finding is that whether the two changes compose additively is itself unstable across runs of the same `RandomizedSearchCV`-based search; a joint re-tune validated on a repeated-split methodology would be needed before trusting either outcome. (§9.7)
27. Construct the argument for why Decision Tree's `max_features="log2"` (rather than the default `None`, i.e. consider every feature) could hurt as easily as help. → Narrower feature subsets per split increase tree diversity within an ensemble, but Decision Tree is a *single* tree here, not an ensemble — restricting candidate features per split has no diversification benefit to offset against, only the risk that a genuinely strong feature isn't offered as a candidate at a given split, which is a straightforwardly worse trade for a lone tree than for a forest. (§7.2)
...
29. Explain why `training_summary.json`'s single point-accuracy numbers cannot, by themselves, tell you whether Random Forest's lead over XGBoost on raw accuracy would replicate on a new random split. → No confidence interval or repeated-split variance estimate exists; a ~1.3-point gap on one 609-row split is consistent with, but not proof against, being within normal split-to-split noise. (§16.2, §18.11)
...
- [ ]  Can state, unprompted and without being asked "what's wrong with your project," at least three genuine weaknesses: class-imbalance correction is inconsistent (only Decision Tree has it, and even that doesn't fix the problem) despite finding a working fix for XGBoost in its own notebook, undocumented hyperparameters for three of the four models, no cross-validation on the final reported numbers (§16)
